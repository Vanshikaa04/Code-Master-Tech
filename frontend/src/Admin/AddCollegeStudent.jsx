...
const AddCollegeStudent= () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res1 = await axios.get(`${backendurl}/api/degree/listdegree`);
        if (res1.data.success && Array.isArray(res1.data.degrees)) {
          setdegrees(res1.data.degrees);
        } else {
          setdegrees([]);
        }
 ...
 setBatches( res2.data.batches.filter((c)=>c.batchof=="College") || []);        }
      } catch (err) {
        console.error(err);
  ...
  const matchingBatches = batches.filter((batch) =>
    batch.courses.includes(degree_name)
  );

  const handleSubmit = async () => {
    try {
      const validateRes = await axios.post(`${backendurl}/api/college/validatestudent`, {
        email,
        phone,
        password,
        confpassword,
      ...
      if (validateRes.data.success) {
        ...
        const registerRes = await axios.post(`${backendurl}/api/admin/addcollegestudent`, formDatastudent ,{headers:{token}});
        ...
        if (registerRes.data.success) {
          ...
          if (addToBatch.data.success) {
            toast.success("College Student Added and Assigned to Batch");
            navigate("/admin");
          } else {
    ...
    <Container fluid className="d-flex align-items-center justify-content-center vh-100">
      <Card className="p-4 shadow" style={{ width: "100%", maxWidth: "700px" }}>
        ...
        <Form>
          ...
          <Row>
            <Col md={6}>
              <Form.Group className="mb-3" controlId="formdegree_name">
                <Form.Label>Select degree</Form.Label>
                <Form.Select value={degree_name} onChange={(e) => setdegreeName(e.target.value)} required>
                  <option value="">Select degree</option>
                  {degrees.map((item) => (
                    <option key={item._id} value={item.degree_name}>
                      {item.degree_name}
                    </option>
                  ))}
                </Form.Select>
            ...
            <Col md={6}>
              <Form.Group className="mb-3" controlId="formBatch">
                ...
                <Form.Select value={selectedBatch} onChange={(e) => setSelectedBatch(e.target.value)} required disabled={!degree_name}>
...
export default AddCollegeStudent;;